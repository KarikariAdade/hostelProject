...
if (isset($_POST['display_image_section'])) {
	...
	if(!empty($hostel_name) && !empty($hostel_status) && !empty($hostel_address) && !empty($hostel_room_type) && !empty($hostel_min_price) && !empty($hostel_max_price) && !empty($hostel_detail)){
		...
		$add = $conn->query("INSERT INTO hostel (manager_id, name, address, status, room_type, features, price_range, description) VALUES('$id', '$hostel_name', '$hostel_address', '$hostel_status', '$hostel_room_type', '$features', '$price_range','$hostel_detail')") or die(mysqli_error($conn));
		if ($add) {
			echo "Hostel submitted, Please proceed to adding Images";
		}
	}else{
